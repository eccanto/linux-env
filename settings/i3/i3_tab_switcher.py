...
class I3Workspaces:
    ...
    def focus_first(self, node: Con) -> None:
        if node.type != 'workspace':
            while node.focus:
                node = self.i3.get_tree().find_by_id(node.focus[0])
            ...
            if len(focused.parent.nodes) > 1:
                ...
                if parent_nodes_length > 1:
                    if position < parent_nodes_length - 1:
                        self.focus_first(self.i3.get_tree().find_by_id(parent_nodes[position + 1]))
                    else:
                        if len(focused.nodes) > 1:
                            self.focus_first(focused)
                        else:
                            self.focus_first(self.i3.get_tree().find_by_id(parent_nodes[0]))
                else:
                    self.i3.command('focus parent')
                    self.focus_next_tab()
            else:
    ...
    def focus_next_group(self, focused: Con = None) -> None:
        ...
        while (
            focused.parent and
            focused.parent.type != 'workspace' and
            focused.parent.layout not in ('splith', 'splitv')
        ):
